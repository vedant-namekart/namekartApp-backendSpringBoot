package com.namekartapp.namekartappapi.dynadot.list;

import com.google.gson.Gson;
import com.namekartapp.namekartappapi.AuctionList;
import org.springframework.web.socket.CloseStatus;
import org.springframework.web.socket.TextMessage;
import org.springframework.web.socket.WebSocketSession;
import org.springframework.web.socket.handler.TextWebSocketHandler;

import java.util.ArrayList;
import java.util.List;

public class DynadotListWebSocketHandler extends TextWebSocketHandler {
    private static final List<WebSocketSession> sessions = new ArrayList<>();
    private static final Gson gson = new Gson();

    @Override
    public void afterConnectionEstablished(WebSocketSession session) {
        sessions.add(session);
        System.out.println("New Dynadot session established: " + session.getId());
    }

    // Modify this method to handle a list of auctions, not just a single auction
    public static void sendAuctionUpdates(List<AuctionList> auctions) {
        String auctionsJson = gson.toJson(auctions);
        for (WebSocketSession session : sessions) {
            try {
                session.sendMessage(new TextMessage(auctionsJson));
                System.out.println("Sent auction updates.");
            } catch (Exception e) {
                System.err.println("Error sending Dynadot auction update: " + e.getMessage());
            }
        }
    }

    @Override
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        sessions.remove(session);
        System.out.println("Dynadot session closed: " + session.getId());
    }
}
